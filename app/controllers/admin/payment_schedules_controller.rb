module Admin
  ...
  class PaymentSchedulesController < BaseController
    def index
      ...
      purchase_bills = current_tenant.purchase_bills
                                     .includes(:supplier)
                                     .joins(:supplier)
                                     .merge(Supplier.search(search_keyword))
                                     .merge(Supplier.with_status(search_status))
    ...
    def build_rows(purchase_bills)
      purchase_bills.filter_map do |purchase_bill|
        next unless schedule_scope_match?(purchase_bill.due_date)

        {
          purchase_bill: purchase_bill,
          supplier: purchase_bill.supplier,
          due_date: purchase_bill.due_date,
          outstanding_amount: purchase_bill.outstanding_amount,
          days_delta: (purchase_bill.due_date - as_of_date).to_i
        }
      end
    end
