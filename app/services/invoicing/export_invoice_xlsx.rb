module Invoicing
  class ExportInvoiceXlsx < Reports::BaseXlsx
    ...
    def build_rows
      ...
      rows << label_value_row("得意先", invoice.customer.name, "支払期日", jp_date(invoice.due_date), "請求締め", invoice.billing_batch&.batch_number || "-")
      rows << label_value_row("請求送付方法", invoice_delivery_method_label(invoice.invoice_delivery_method_snapshot), "支払条件", payment_due_rule_label(invoice.payment_due_rule_snapshot), "締日", "#{jp_date(invoice.closing_date)} (#{invoice.closing_day_snapshot || "-"}日締め)")
      rows << label_value_row("得意先担当者", invoice.customer.primary_contact.presence || "-", "メール", invoice.customer.contact_person_email.presence || invoice.customer.email.presence || "-", "電話", invoice.customer.contact_person_tel.presence || invoice.customer.tel.presence || "-")
      rows << full_width_value_row("請求期間", "#{jp_date(invoice.billing_period_from)} 〜 #{jp_date(invoice.billing_period_to)}", columns: column_widths.size)
      rows << full_width_value_row("請求先住所", invoice.customer.full_address.presence || "-", columns: column_widths.size)
      rows << full_width_value_row("備考", invoice.remarks.presence || "-", columns: column_widths.size)
