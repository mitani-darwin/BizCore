module Purchases
  class ExportPurchaseOrderXlsx < Reports::BaseXlsx
    ...
    def build_rows
      ...
      rows << label_value_row("仕入先", purchase_order.supplier.name, "希望納期", jp_date(purchase_order.requested_delivery_date), "発注担当者", purchase_order.ordered_by_name.presence || "-")
      rows << label_value_row("仕入先担当者", purchase_order.supplier.primary_contact.presence || "-", "メール", purchase_order.supplier.contact_person_email.presence || purchase_order.supplier.email.presence || "-", "電話", purchase_order.supplier.contact_person_tel.presence || purchase_order.supplier.tel.presence || "-")
      rows << full_width_value_row("入荷倉庫", purchase_order.warehouse.name, columns: column_widths.size)
      rows << full_width_value_row("備考", purchase_order.remarks.presence || "-", columns: column_widths.size)
      rows << blank_row
